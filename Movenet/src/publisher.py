...
class Publisher(mqtt.Client):
    ...
    # send data to Unity
    def send(self, username, player_pos):
        topic = f"topic/pose/{username}"
        if player_pos == Position.GAME_START:
            self.publish(topic, "g", qos=1)
        elif player_pos == Position.JUMP_START:
